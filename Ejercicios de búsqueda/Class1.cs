...
internal class Programa
{
    ...
    static int Primerevento(int[] arr)
    {
        ...
        while (left <= right)
        {
            ...
            if (arr[mid] % 2 == 0)
            {
                ...
                if (mid == 0 || arr[mid - 1] % 2 != 0)
                    return mid; // Devuelve la primera ocurrencia de un número par.
                else
                    right = mid - 1; // Busca en la mitad izquierda para encontrar el primero.
            }
            else
            {
                left = mid + 1;
        ...
        return -1; // No se encontraron números pares.
    }
